...
var ɵ0 = function ($) {
    ...
    var Dropdown = (function () {
        ...
        Dropdown.prototype._getConfig = function (config) {
            config = __assign({}, this.constructor.Default, $(this._element).data(), config);
        ...
        Dropdown.prototype._getMenuElement = function () {
            if (!this._menu) {
                ...
                this._menu = $(parent_1).find(Selector.MENU)[0];
            }
            return this._menu;
        };
        ...
        Dropdown.prototype._getPopperConfig = function () {
            ...
            if (typeof this._config.offset === 'function') {
                offsetConf.fn = function (data) {
                    data.offsets = __assign({}, data.offsets, _this._config.offset(data.offsets) || {});
                    return data;
                };
            }
            else {
        ...
        Dropdown._clearMenus = function (event) {
            ...
            for (var i = 0; i < toggles.length; i++) {
                ...
                if (!$(parent_2).hasClass(ClassName.SHOW)) {
                    continue;
                }
                if (event && (event.type === 'click' &&
                    /input|textarea/i.test(event.target.tagName) || event.type === 'keyup' && event.which === TAB_KEYCODE) &&
                    $.contains(parent_2, event.target)) {
                    continue;
                }
                var hideEvent = $.Event(Event.HIDE, relatedTarget);
                $(parent_2).trigger(hideEvent);
